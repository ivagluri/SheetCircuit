...
LOG = os.path.join(os.path.dirname(os.path.abspath(__file__)), "play2_transcript.log")
...
for i in range(30):
    ...
    active_car = STARTER_CAR if level < 2 else RACE_CAR
    try:
        out = run_race(ev, active_car)
    ...
    if level >= 3:
        ...
        break
    if 0 <= condition < 70:
        out = repair(active_car)
        money, _, _ = status(out)
        note(f"repaired {active_car}, ${money}")
else:
