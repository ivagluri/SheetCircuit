...
LOG = os.path.join(os.path.dirname(os.path.abspath(__file__)), "play4_transcript.log")
...
for i in range(25):
    ...
    if level >= 3:
        note(f"*** TEAM LEVEL 3 REACHED after {races} more races (${money}, week shown above) ***")
        break
    if money < 300:
        flag(f"broke: ${money}")
        break
