...
LOG = os.path.join(os.path.dirname(os.path.abspath(__file__)), "play5_transcript.log")
...
races, phase, repaired_d = 0, "farm", False
for i in range(30):
    if phase == "farm" and money >= 2900:
        out = repair("nagoya_march_super")
        money, _, _ = status(out)
        if "Insufficient" not in out:
            note(f"repaired nagoya to D class, ${money}")
            phase, repaired_d = "dwin", True
    ev, car = ("open_track_day", "eurovan_cup") if phase == "farm" else (("clubman_trial", "nagoya_march_super") if i % 2 == 0 else ("oval_night", "nagoya_march_super"))
    try:
        out = run_race(ev, car)
